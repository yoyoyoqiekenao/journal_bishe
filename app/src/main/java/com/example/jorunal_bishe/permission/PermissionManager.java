package com.example.jorunal_bishe.permission;
    ...
    private void request(Object object, String[] permissions, int requestCode) {
        ...
        if (deniedPermissions.size() > 0) {
            ...
            if (rationales.size() > 0 && mIsPositive == false) {
                if (mListener != null ) {
                    Log.i(TAG, "request onShowRationale");
...
//        }
    ...
    public void onPermissionResult(String[] permissions, int[] results) {
        ...
        if (deniedPermissions.size() > 0) {
            if (mListener != null) {
                Log.i(TAG, "onPermissionResult onDenied");
                mListener.onDenied();
            }
        } else {
            if (mListener != null) {
                Log.i(TAG, "onPermissionResult onGranted");
                mListener.onGranted();
            }
        }
    }
